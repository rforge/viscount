VisCount <- function(Nmax = 100, Nmin = 5, shape = 20, size = 1, col = "black", bg = "lightblue") {
  
  if (length(grep(".png", c(shape, bg)) > 0) & !("png" %in% rownames(installed.packages())))  return(message("Using .png images requires the 'png' R package - please install it and try again."))
  ...
  plot.min <- 0
  plot.max <- ifelse(length(grep(".png", shape)) > 0, 1 + size, 1)  # so images fit in plot
